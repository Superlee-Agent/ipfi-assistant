...
export async function swapAgent(request: SwapRequest): Promise<SwapAgentResponse> {
  try {
    if (request.kind === "prompt") {
      const res: PromptResult = await handlePrompt(request.text, request.taker);
      if (res.type === "swap.intent.invalid") return { ok: false, error: res.message };
      if (res.type === "swap.intent") {
        const quote = await getQuote({ ...res.intent, taker: request.taker });
        const message = humanize(res.intent.chain, res.intent.sellToken, res.intent.buyToken, res.intent.amount, res.intent.side);
    ...
    const intent: SwapIntent = { chain, sellToken, buyToken, amount, side: side ?? "sell" };
    const quote = await getQuote({ chain, sellToken, buyToken, amount, taker, side });
    const message = humanize(chain, sellToken, buyToken, amount, side ?? "sell");
