...
export const AENEID_V3_QUOTER =
  process.env.AENEID_V3_QUOTER ?? "0x8812d810EA7CC4e1c3FB45cef19D6a7ECBf2D85D";
...
export async function quoteExactInputSingle(
  ...
  if (!providerUrl)
    throw new Error("Missing STORY_RPC env var for Story testnet RPC URL");
  ...
  const [amountOut, sqrtPriceX96After, initializedTicksCrossed] =
    await quoter.quoteExactInputSingle(tuple);
