...
export async function handlePrompt(prompt: string, taker?: string): Promise<PromptResult> {
  const intent = parseSwapPrompt(prompt);
  if (!intent) return { type: "swap.intent.invalid", message: "Prompt tidak jelas. Contoh: 'Swap 100 USDC ke ETH di Base'." };
